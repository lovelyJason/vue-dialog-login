<template>
  <div>
    <!-- 弹出层 -->
    <!-- 显示时添加类名md-show -->
    <div :class="['md-modal',`md-effect-${type}`,flagObj.flag?'md-show':'']" :id="'modal-'+type">
      <div class="md-content">
        ...
        <div>
          ...
          <button class="md-close" @click="close">Close me!</button>
        </div>
      </div>
    </div>
    <!-- 遮挡层 -->
...
.md-overlay {
  ...
  z-index: 1000;
  opacity: 0;
  background: rgba(122, 155, 153, 0.8);
  -webkit-transition: all 0.3s;
  -moz-transition: all 0.3s;
  transition: all 0.3s;
}
/* 这里兄弟选择器当md-show出现时,即会显示遮挡层,
可以不用给遮挡层进行逻辑处理 */
.md-show ~ .md-overlay {
  opacity: 1;
  visibility: visible;
